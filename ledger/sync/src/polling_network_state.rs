...
// Ideally this should not be a node id that can be used on a real network.
...
pub struct PollingNetworkState<BC: BlockchainConnection> {
    ...
    /// SCPNetworkState instance that provides the actual blocking/quorum set check logic.
...
impl<BC: BlockchainConnection + 'static> PollingNetworkState<BC> {
    pub fn new(
        ...
        // Since we want to re-use the findQuorum method on our QuorumSet object, fabricate a
        // message map based on the current block indexes we're aware of.
...
impl<BC: BlockchainConnection> NetworkState for PollingNetworkState<BC> {
    /// Returns true if `connections` forms a blocking set for this node and, if the local node is
    /// included, a quorum.
    ///
    /// # Arguments
